...
var mockSearchObject = {name: mockName, age: 15}
var mockSearchArray = [mockSearchObject];
...
describe('Searchanator', function () {
	...
	describe('Searcing by string', function () {
		describe('With a search term of ' + mockSearchTerm, function () {
			...
			describe('With multiple arrays', function () {
				it('Should return the desired objects', function () {
					expect(search(mockSearchTerm, mockOptions, mockSearchArray, [])[0].name).to.equal(mockName);
					expect(search(mockSearchTerm, mockOptions, mockSearchArray, []).length).to.equal(1);
				});
			});
			describe('With a multidimensional array', function () {
				it('Should return a single array containing all results', function () {
					debugger;
	...
	describe('Speed Test', function () {
		var speedTestArray = [];
		var size = 100000;
		var middleOfArray = size / 2;
		var options = {
			getSearchTerm: x => x.id
		};
		for (var i = 0; i < size; i++) {
			speedTestArray.push({id: i});
		}
		it('Should be fast', function () {
			function calcDiffDate(before, after){
				var dayInMillies = 1000*60*60*24;
				var beforeMs = before.getTime();
				var afterMs = after.getTime();
				var diff = afterMs - beforeMs;
				return Math.round(diff/dayInMillies);
			}
			var before = new Date();
			search(middleOfArray, options, speedTestArray);
			var after = new Date();
			expect(calcDiffDate(before,after)).to.equal(0);
		});
